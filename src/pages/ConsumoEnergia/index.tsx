...
export default function Home() {
  ...
  const  maxLengthCheck = (maxLength: number) => {
    const wattsValue = watts.toString()
    if (wattsValue.length > maxLength) {
      setWatts(+wattsValue.slice(0, maxLength));
      console.log(+wattsValue.slice(0, maxLength),' value')
      console.log(watts, ' watss')
    }
  }
      ...
      <div className={styles.wrapper}>
        <div className={styles.inputDetails}>
          <input
            ...
            disabled={time > 0}
            value={watts ? watts : ""}
            onKeyUp={() => maxLengthCheck(5)}
            onChange={(e: React.FormEvent<HTMLInputElement>) => {
              setWatts(+e.currentTarget.value);
              setIsRequired(false);
            }}
          />
          <img
